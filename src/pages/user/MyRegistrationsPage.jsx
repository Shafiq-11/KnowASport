...
import { Ticket, Calendar, MapPin, ArrowRight, Clock, AlertCircle } from 'lucide-react';
    ...
    <div className="kas-container py-8 lg:py-12 space-y-8">
      ...
      <div className="border-b border-neutral-200 pb-6">
        ...
        <p className="text-sm text-neutral-500">
          Manage your tournament passes, digital tickets, and registration statuses
        </p>
      </div>
        ...
        >
          {filtered.map((reg) => (
            <motion.div
              ...
              className="bg-white rounded-[16px] border border-neutral-200 p-6 space-y-4 shadow-sm hover:shadow-md transition-shadow"
            >
              <div className="flex items-start justify-between gap-2 border-b border-neutral-100 pb-3">
                <div>
                  <span className="text-[11px] font-800 text-neutral-400 font-mono block uppercase">
                    {reg.registration_number}
                  </span>
                  <span className="text-xs font-700 text-amber-600 uppercase tracking-wide">
                    {reg.event?.sport_name || 'Sports'}
                  </span>
                </div>

                <Badge
                  variant={reg.status === 'confirmed' ? 'success' : reg.status === 'cancelled' ? 'danger' : 'warning'}
                  size="sm"
                >
              ...
              <div className="pt-3 border-t border-neutral-100 flex items-center justify-between gap-4">
                <div className="flex flex-col">
                  <span className="text-[11px] text-neutral-400 font-600">Payment Status</span>
